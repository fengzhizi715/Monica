# Monica
Monica 是一款跨平台的桌面图像编辑软件，使用 Kotlin Compose Desktop 作为 UI 框架。应用层是由 Kotlin 编写的，基于 mvvm 架构，使用 koin 作为依赖注入框架。
大部分算法用 Kotlin 编写，少部分算法通过 jni 调用 OpenCV C++ 实现的图像处理或调用深度学习的模型。
